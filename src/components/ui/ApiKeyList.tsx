...
export function ApiKeyList({ value, onChange, max = 16 }: Props) {
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full">
      {value.length === 0 && (
        <div className="text-xs text-text-3 italic px-1 py-2">
          No API keys yet. Click "Generate" to create one.
        </div>
      )}
      {value.map((k, i) => (
        <div key={i} className="flex items-center gap-1.5 w-full">
          ...
          <button
            ...
            className="inline-flex items-center justify-center w-7 h-7 rounded-md border border-border bg-raised text-text-3 hover:text-text-1 hover:bg-hover transition-colors shrink-0"
          >
            {copiedIdx === i ? (
          ...
          <button
            ...
            className="inline-flex items-center justify-center w-7 h-7 rounded-md border border-border bg-raised text-text-3 hover:text-err hover:border-err-border transition-colors shrink-0"
          >
      ...
      <div className="flex items-center gap-2 mt-1">
        ...
        {value.length >= max && (
          <span className="text-[10px] text-text-3">Max {max} keys</span>
        )}
      </div>
    </div>
  )
